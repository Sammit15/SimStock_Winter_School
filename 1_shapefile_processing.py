...
file_name = 'test_fixed_EPSG32643' #shapefile name
...
def expand_floors(df):
    ...
    mask1 = (df['use'] == 'Mixed Use') & (df['floor_no']==1)
    df['use'] = df['use'].mask(mask1,'Ret_Sales')
    mask2 = (df['use'] == 'Mixed Use') & (df['floor_no']>1)
    df['use'] = df['use'].mask(mask2,'Residential')
    return(df)

def combine_uses(df):
    #combine uses to simplify model
    df.replace({'Commercial': 'Office', 'Religious': 'Education',
        'Health':'Office','Public Service':'Office'}, inplace = True)
    return df
...
def preprocessing_constructions():
    ...
    def constructions():
        ...
        improved = {construction_elements[0]: 'improved_wall',
                            ...
                            construction_elements[3]: 'ceiling',
                            construction_elements[4]: 'ceiling_inverse',
    ...
    def random_constructions(row, construction_sets,
                             construction_elements, construction_dict):
        if pd.isnull(row['AMD_glazing_ratio']):
            row['AMD_glazing_ratio'] = int(10 * rd.randint(1, 9))
        if row[construction_elements].isnull().values.any():
            const = rd.choice(construction_sets)
            for item in construction_elements:
                row[item] = construction_dict[const][item]
        return row
